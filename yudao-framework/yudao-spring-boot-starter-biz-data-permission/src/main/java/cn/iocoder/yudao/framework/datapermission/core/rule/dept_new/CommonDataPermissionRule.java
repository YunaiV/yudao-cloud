...
/**
 * 基于 （租户/部门/用户）关联表 的 {@link DataPermissionRule} 全局共享数据权限 规则实现
 *
 * 注意，使用 DeptDataPermissionRule 时，需要保证 存在关联关系表 且关联关系表中存在 user_id dept_id tenant_id 字段
 * 注意此处授权时，授权逻辑请注意：
 *  1. 授权 租户/部门/用户 时，会新增 数据主键和 租户/部门/用户 主键 关联关系时，尽可能多的创建数据不要将 dept_id 和 user_id 字段 都塞到同一条数据中，(租户字段可以塞)
 *  2. 新增 数据主键和部门/用户主键 关联关系，需要手动实现
 ...
 *  以上三种 最后一个 ? 替换为原始的 dept_id 和 user_id 字段,如需增加租户字段可在此处增加。
 * @author sur1-123
 */
@AllArgsConstructor
@Slf4j
public class CommonDataPermissionRule implements DataPermissionRule {
    ...
    class DataPermissionConf {
        ...
        //  user_channel为关联表 通道编码对应
        //  数据权限字段 user_id dept_id tenant_id
        //  数据关系表user_channel
    ...
    public Expression getExpression(String tableName, Alias tableAlias) {
        ...
        // 情况一，如果是 ALL, 并且为系统租户 可查看全部，则无需拼接条件
        ...
        Expression deptExpression = buildDeptExpression(tableName, tableAlias, deptDataPermission.getDeptIds(), loginUser.getTenantId());
        Expression userExpression = buildUserExpression(tableName, tableAlias, deptDataPermission.getSelf(), loginUser.getId(), loginUser.getTenantId());
    ...
    private Expression buildDeptExpression(String tableName, Alias tableAlias, Set<Long> deptIds, Long tenantId) {
        // 如果不存在配置，则无需作为条件
        ...
        PlainSelect selectBody = (PlainSelect) SelectUtils.buildSelectFromTableAndExpressions(
                new Table(columnRule.getDataPermissionTableName())
                , new Column(columnRule.getDataPermissionFieldName())).getSelectBody();
        selectBody.setWhere(new AndExpression(new InExpression(
                ...
                new EqualsTo(MyBatisUtils.buildColumn(columnRule.getDataPermissionTableName(), null, "tenant_id"), new LongValue(tenantId))
        ));
        return new InExpression(
                MyBatisUtils.buildColumn(tableName, tableAlias, columnRule.getTableFieldName()),
                new Parenthesis(selectBody)
    ...
    private Expression buildUserExpression(String tableName, Alias tableAlias, Boolean self, Long userId, Long tenantId) {
        // 如果不查看自己，则无需作为条件
        ...
        selectBody.setWhere(new AndExpression(new EqualsTo(MyBatisUtils.buildColumn(columnRule.getDataPermissionTableName(), null, columnRule.getDataPermissionJudgementFieldName())
